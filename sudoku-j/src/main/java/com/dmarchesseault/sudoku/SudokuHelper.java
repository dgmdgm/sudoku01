...
class SudokuHelper implements SudokuConstants
{
    ...
    {
        LOG.debug("populateBoardFromText: begin");
...
//                board.changeCellDigit(i, digit, CellChange.Reason.INIT);
        ...
        LOG.debug("populateBoardFromText: end");
        return board;
    }
    ...
    static void populateBoardRandomly2(final Board targetBoard, final int n, final long seed)
    {
        ...
        try
        {
            SudokuHelper.populateBoardFromText(sourceBoard, SudokuSamples.SUDOKU_BOARD_CANONICAL2);
        }
        catch (BoardSanityException e)
        {
            // should not happen with jnown good board
            //todo handle exception
            //e.printStackTrace();
        }

        // Make random permutation
        final int[] pRow = makeRowPermutation(seed);
        final int[] pCol = makeRowPermutation(seed+13);

        // To avoid collisions from cell index reuse, we use a list of indices and remove them randomly until
        // we have just enough
        final List<Integer> cellChoices = new ArrayList<Integer>(81);
        for (int i = 0; i < 81; i++) cellChoices.add(i);
        while (cellChoices.size() > n)
        {
            final int cellChoice = (int)(random.nextDouble() * cellChoices.size());
            cellChoices.remove(cellChoice);
        }

        for (int cellIndex : cellChoices)
        {
            final Cell sourceCell = sourceBoard.getCell(cellIndex);
            final int sourceRow = sourceCell.getRow();
            final int sourceCol = sourceCell.getCol();
            final int targetRow = pRow[sourceRow];
            final int targetCol = pCol[sourceCol];

            try
            {
                final int sourceDigit = sourceCell.getDigit();

                targetBoard.getCell(targetRow,targetCol).changeToDigit(sourceDigit, CellChange.Reason.INIT);
            }
            catch (BoardSanityException e)
            {
     ...
     * @param cellFilter optional filter of cell to keep
     */
    static void populatePermutatedBoard(final Board targetBoard, final Board sourceBoard,
                                        final int[] rowPermutation, final int[] colPermutation,
                                        final Predicate<Cell> cellFilter)
    {
        ...
        for (int sourceRow = 0; sourceRow < 9; sourceRow++)
        {
            for (int sourceCol = 0; sourceCol < 9; sourceCol++)
            {
                ...
                if (cellFilter == null || cellFilter.test(sourceCell))
                {
                    try
                    {
                        final int sourceDigit = sourceCell.getDigit();
    ...
    static int[]  makePermutation(final int n, long seed)
    {
        ...
        final List<Integer> choices = new ArrayList<Integer>(n);
     ...
     * @return Pair of permutations for rows and columns. Null means failure.
     */
    static Pair<int[], int[]> findBoardPermutation(final Board sourceBoard, final Board targetBoard)
    {
...
//            throw new IllegalArgumentException("Size must be 9");
        ...
        {
            final int targetGroupIndex0 = p[sourceGroupIndex * 3] / 3;
