console && console.log("### Set.js begin");

// Implementation borrowed from O'Reily's JavaScript The Definitive Guide

(function(ns)
{

    ns.Set = function() {                   // This is the constructor
    ...
    ns.Set.prototype.add = function() {
        for(var i = 0; i < arguments.length; i++) {  // For each argument
            var val = arguments[i];                  // The value to add to the set
            var str = ns.Set._v2s(val);                 // Transform it to a string
    ...
    ns.Set.prototype.remove = function() {
        for(var i = 0; i < arguments.length; i++) {     // For each argument
            var str = ns.Set._v2s(arguments[i]);         // Map to a string
    ...
    ns.Set.prototype.contains = function(value) {
        return this.values.hasOwnProperty(ns.Set._v2s(value));
    };

    // Return the size of the set.
    ns.Set.prototype.size = function() { return this.n; };

    // Call function f on the specified context for each element of the set.
    ns.Set.prototype.foreach = function(f, context) {
    ...
    ns.Set._v2s = function(val) {
        ...
        function objectId(o) {
            ...
            if (!o.hasOwnProperty(prop))   // If the object has no id
                o[prop] = ns.Set._v2s.next++; // Assign it the next available
            return o[prop];                // Return the id
        }
    };
    ns.Set._v2s.next = 100;    // Start assigning object ids at this value.

}(Module("util")));

console && console.log("### Set.js end");
