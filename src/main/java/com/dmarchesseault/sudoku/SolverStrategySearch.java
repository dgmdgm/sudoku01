...
class SolverStrategySearch extends SolverStrategy
{
            ...
            catch (BoardSanityException e)
            {
                ...
                while (bse != null) {
                    try
                    {
                        ...
                        // We disallow picking the next digit for pure search, otherwise we risk excetion within this catch.
    ...
    int applyOnce(Board board) throws BoardSanityException
    {
        ...
        /// Fewer candidates should improve the odds of a good guess are better and a smaller search tree.

        final Cell[] cells = board.getCells();
        Cell bestCell = null;
        int bestDigitsFlagsCount = 9+1;
        int unknownDigitCells = 0;
        for (Cell cell : cells)
        {
            ...
            {
                LOG.warn("scanIdenticalFlagsInGroup: Integrity problem; no flags in cell " + cell);
            }
        }
            ...
            else
            {
                ...
                if (cellChange.getReason() == CellChange.Reason.SOLVER_GUESS)
                {
                    ...
                    if (digitFlagsLessGuess != 0)
                    {
                        ...
                        {
                            // Just turn off the dlag for the failed guess
